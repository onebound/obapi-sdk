...
if(substr($url,0,4)!='http'){		
        ...
        if (strpos($url, 'taobao.') !== false || strpos($url, 'tmall.') !== false) {
            ...
            preg_match('/1688\.com.*?(offerId=|itemId=|offer\/|offerId\/)(\d+)/is', $url, $match);
            $result['id'] = !empty($match[2]) ? $match[2] : '';
            ...
            if(empty($match)){
                preg_match('@jd.hk/(\d+).html@isU', $url, $match);
            }
            if (!isset($match[1])) {
			...
			// $result['type']='vvic';
			//使用item/vvic.php 处理
		}elseif (strpos($url,'.vip.com')!==false) {
            // https://detail.vip.com/detail-1710613505-6918151870211446721.html
            $result['id']   = explode('.',explode('detail-',$url)[1])[0];
            $result['type'] = 'vip';
        }elseif (strpos($url,'weidian.com')!==false) {
            // https://weidian.com/item.html?itemID=2749499386
            $url_list = parse_url($url);
            $query_info = explode("&",$url_list['query']);
            $numId = 0;
