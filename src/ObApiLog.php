...
';
		...
		echo '<form action="?" id="log_list"><button class="btn btn-xs"  title="定制显示列" onclick="grid_custom()" type="button">定制显示列</button><input type="reset"><input type="submit" value="搜索"><input type="hidden" name="debug" value="view_log">';
		
		echo $this->client->helper_obj->outtable($logs);
		echo '</form>';
		ECHO '</div></fieldset>';
		echo "<script>$('#log_list table tr:eq(1) td').each(function(i){
    $(this).html($(this).text()); return;
})</script>";
		exit();
	...
	public function write($log){
		...
		if(strlen($log['api_result'])>65000){
 			$log['api_result'] = substr($log['api_result'] ,0,65000).'...('.strlen($log['api_result']).')';
		}
