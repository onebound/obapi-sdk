...
class ObApiClient
{
    ...
    public function curl($url, $post = null)
    {
        ...
        if ($this->api_name == 'item_password') {
            $this->m_curl->setopt(CURLOPT_TIMEOUT, 1);
        } else {
            $this->m_curl->setopt(CURLOPT_TIMEOUT, 15);
        }
        $this->m_curl->setopt(CURLOPT_HEADER, 0);
     ...
     * 获取跳转网址
     */
    public function file_get_rurl($html = '')
    {
        $url = '';
    ...
    public function execute($request, $session = null)
    {
        ...
        try {
            ...
            if (!$data && $this->cache_local) {
                ...
                if ($this->api_l_status) {
                    ...
                    if (is_array($result) && !empty($result['server_time']) && empty($result['error']) && empty($result['item']['error'])) {
            ...
            $this->API_db_log(array(
                ...
                'api_request' => substr($post2log, 0, 255),
                'api_result' => substr($this->api_text, 0, 255),
                'guest_ip' => $this->ip(),
                'guest_host' => $_SERVER['HTTP_HOST'],
                'guest_agent' => $guest_agent,
                'guest_post' => substr($guest_post, 0, 255),
                'guest_url' => $guest_url,
                'guest_referer' => substr($guest_referer, 0, 255),
        ...
        $execute_n++;
        $result["post_url"] = '';
        if (isset($_GET['debugsdk']) && $_GET['debugsdk'] = 1 && $this->api_name == 'item_get') {
            var_dump($result['item']['props_img']);
            exit;
        }
        return $result;
    }
